...
export default function Soriziller() {
  ...
  useEffect(() => {
    ...
    if(volume >= 0 && volume <= 25){level<1 && setLevel(1)} // 1
    else if(volume >= 26 && volume <= 35){level<2 && setLevel(2)} // 2
    else if(volume >= 36 && volume <= 45){level<3 && setLevel(3)} // 3
    else if(volume >= 46 && volume <= 60){level<4 && setLevel(4)} // 4
    else if(volume >= 61 && volume <= 120){level<5 && setLevel(5)} // 5
  }, [volume]);
